    ...
    <section className="relative grid md:grid-cols-2 items-center">
          ...
          {id === 3 ? 
            <div className="flex justify-center md:justify-start">
              <img 
                src={underlineSm} 
                alt=''
                className='xl:max-w-none lg:max-w-45 max-w-39 -ml-3.5'
              />
            </div>
            : <img 
              src={underline} 
      ...
      <div className="relative h-full order-1 md:order-0 mx-auto">
        ...
        { img && 
          ...
          className={`${id === 3 ? 'xl:max-h-118' : ''} object-cover object-center h-full w-full`} /> 
        }
        {id === 3 && <img src={divider} alt='' className="hidden md:block absolute md:w-12 lg:w-12.5 top-0 md:right-0 lg:-right-6.25 h-full"/>}
        {id === 3 && <img src={dividersm} alt='' className="md:hidden absolute -bottom-2 w-full"/>}
      </div>
    </section>  
  )
}
